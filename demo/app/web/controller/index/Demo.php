...
class Demo {
	...
	function sendAndroidBroadcast($data) {
		try {
			...
			$brocast->setPredefinedKeyValue("ticker",           "ALARM");
			$brocast->setPredefinedKeyValue("title",            "入侵检测");
			$brocast->setPredefinedKeyValue("text",             $data);
			...
			error_log("Sending broadcast notification, please wait...\r\n");
			$brocast->send();
	...
	function sendAndroidUnicast() {
		try {
			...
			error_log("Sending unicast notification, please wait...\r\n");
			$unicast->send();
			error_log("Sent SUCCESS\r\n");
		} catch (Exception $e) {
			error_log("Caught exception: " . $e->getMessage());
